const CONFIG = {
    ...
    COGNITO: {
        ...
        REDIRECT_URI: (() => {
            try {
                // Use current origin for redirect URI to support multiple domains
                const origin = window.location.origin;
                return new URL('/auth-callback.html', origin).href;
            } catch (e) {
                console.error('Invalid REDIRECT_URI:', e.message);
                return 'https://teckstart.com/auth-callback.html';
            }
        })(),
        SIGN_OUT_URI: (() => {
            try {
                // Use current origin for sign out URI
                const origin = window.location.origin;
                return new URL('/', origin).href;
            } catch (e) {
                console.error('Invalid SIGN_OUT_URI:', e.message);
                return 'https://teckstart.com';
            }
        })()
    }
};
